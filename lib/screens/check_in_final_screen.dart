...
class CheckInFinalScreen extends StatefulWidget {
  static const routeName = '/check_in_final';
...
class _CheckInFinalScreenState extends State<CheckInFinalScreen> {
  bool isLoading = true;
                ...
                builder: (context, authProvider, child) => ListView(
                  ...
                  children: [
                    ...
                    ListTile(
                      tileColor: Colors.white,
                      title:
                          const Text('Lokasi Check In (Latitude, Longitude)'),
                      subtitle: Text(
                      ...
                      child: ElevatedButton.icon(
                        ...
                        label: Consumer<AppStateProvider>(
                          builder: (context, AppStateProvider appStateProvider,
                                  child) =>
                              Text(
                                  ' Check in (${DateFormat('dd MMM yyyy kk:mm:ss').format(appStateProvider.currentDateTime).toString()})'),
                        ),
                      ),
                    )
                  ],
  ...
  Future<void> _submit() async {
    ...
    if (u != null) {
      ...
      http.StreamedResponse response = await ApiServices().checkIn(
        context: context,
        pegawaiId: user.pegawai.id,
        bidangId: user.pegawai.bidangId ?? 0,
        checkedInAt: currentDateTime.toString(),
        checkedInLatitude: widget.locationData.latitude ?? 0,
        checkedInLongitude: widget.locationData.longitude ?? 0,
